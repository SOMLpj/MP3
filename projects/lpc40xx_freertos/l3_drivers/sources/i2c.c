...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  enum {
    ...
    I2C__STATE_SR_MASTER_ADDR_ACK = 0x60,       // Slave acknowledges master is in transmit mode
    I2C__STATE_SR_MASTER_DATA_ACK = 0x80,       // Slave acknowledges data transmitted by Master
    I2C__STATE_SR_MASTER_LAST_DATA_NACK = 0x88, // Slave acknowledges last byte written by Master

    // Slave Transmitter States(MR):
    I2C__STATE_ST_SLAVE_ADDR_ACK = 0xA8,        // Slave acknowledges master is in receive mode
    I2C__STATE_ST_MASTER_DATA_ACK = 0xB8,       // Slave transmits data to master
    I2C__STATE_ST_MASTER_LAST_DATA_NACK = 0xC0, // Slave transmits last byte to master
    I2C__STATE_ST_SLAVE_LAST_DATA_SENT = 0xC8,  // Slave last byte is sent to master

    // BOTH SR and ST
    I2C__STATE_SR_ST_STOP_OR_RESTART = 0xA0 // SET THE STATUS REG HERE?
  };
  ...
  case I2C__STATE_SR_MASTER_ADDR_ACK: // 0x60
                                      // //**********************************************************************************************************************
    i2c__set_ack_flag(lpc_i2c);       // SEND ACK
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;
  ...
  case I2C__STATE_MT_SLAVE_DATA_ACK:
    ...
    if (i2c__is_read_address(i2c->slave_address)) {
      ...
      if (0 == i2c->number_of_bytes_to_transfer) {
        ...
        lpc_i2c->DAT = *(i2c->output_byte_pointer); // SEND THE FIRST BYTE
        ++(i2c->output_byte_pointer);               // POINT TO THE NEXT BYTE OF DATA TO SEND
  ...
  case I2C__STATE_SR_MASTER_DATA_ACK: // 0x80
                                      // //**********************************************************************************************************************
    // lpc_i2c->DAT = *(i2c->input_byte_pointer); // SETTING THE DATA NTH BYTE
    // i2c_slave_callback__write_memory(uint8_t memory_index, uint8_t memory_value); //CALL FUNCTION FOR DATA

    i2c__set_ack_flag(lpc_i2c); // SEND ACK
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;

  case I2C__STATE_SR_MASTER_LAST_DATA_NACK:
    lpc_i2c->DAT = *(i2c->input_byte_pointer); // SETTING THE DATA NTH BYTE
  ...
  case I2C__STATE_ST_SLAVE_ADDR_ACK: // A8
    i2c__set_ack_flag(lpc_i2c);
    break;

  case I2C__STATE_ST_MASTER_DATA_ACK:           // B8
    lpc_i2c->DAT = *(i2c->output_byte_pointer); // SEND THE FIRST BYTE
    //--(i2c->number_of_bytes_to_transfer); MAY NEED THIS FOR FUTURE USE
    i2c__clear_si_flag_for_hw_to_take_next_action(lpc_i2c);
    break;

    // case I2C__STATE_ST_MASTER_LAST_DATA_NACK:
    // DONT KNOW WHAT DO IN HERE?
    // SLAVE RETURN TO IDLE??

  case I2C__STATE_ST_SLAVE_LAST_DATA_SENT:
    lpc_i2c->DAT = 0xF;
    // DONT KNOW WHAT TO DO NEXT? RETURN TO IDLE?
